...
const OrderDetail = () => {
  ...
  useEffect(() => {
    ...
    const fetchOrderDetail = async () => {
      const response = await getOrderDetail(Number(order_id as string));
  ...
  return (
    <>
      <Spring className="card w-11/12 mx-auto h-[65vh]">
          ...
          <div className="flex justify-between w-60">
            ...
            <div>
              <p className="">
                {detail
                  ? numberToVND(
                      (detail.data.end_price - detail.data.discount) as number
                    )
                  : "Đang tải..."}
              </p>
            </div>
